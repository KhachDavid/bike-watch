...
const GameControls: React.FC = () => {
  ...
  const handleNextTurn = () => {
    ...
    setTimeout(() => {
      // Send performance review email if it was generated (January)
      if (nextTurnAction.performanceReviewEmail) {
        dispatch({ type: EMAIL_ACTION_TYPES.ADD_EMAIL, payload: nextTurnAction.performanceReviewEmail });
      }
      ...
      // Send mayor detective praise email if generated
      if (nextTurnAction.mayorDetectiveEmail) {
        dispatch({ type: EMAIL_ACTION_TYPES.ADD_EMAIL, payload: nextTurnAction.mayorDetectiveEmail });
      }
      ...
      if (wasAboveThreshold && !actionsLocked) {
        ...
        // Generate normal social media posts based on current state (including detective mentions!)
        const newPosts = generateNewPosts(currentTurn + 1, streets, detectives, recoveryRate);
